<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase transition-colors">Tambah <span class="text-emerald-600 dark:text-emerald-500 transition-colors">Status</span></h1>
        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest transition-colors">Definisi Siklus Hidup Laporan Baru</p>
    </div>
    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-8 relative overflow-hidden group shadow-sm dark:shadow-2xl transition-colors">
            ...
            <form method="POST" action="{{ route('admin.statuses.store') }}" class="relative z-10 space-y-8">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <select name="is_closing" id="is_closing" required
                        ...
                        <option value="0" {{ old('is_closing', false) == false ? 'selected' : '' }} class="bg-white dark:bg-slate-900">AKTIF (Sedang dalam proses penanganan)</option>
                        <option value="1" {{ old('is_closing', false) == true ? 'selected' : '' }} class="bg-white dark:bg-slate-900">CLOSING (Tahap penyelesaian/selesai)</option>
                    </select>
                    @error('is_closing')
                        <p class="mt-1 text-[10px] font-bold text-red-500 uppercase tracking-tight">{{ $message }}</p>
                    @enderror

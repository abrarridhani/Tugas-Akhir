<x-admin-layout>
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase transition-colors">Rencana <span class="text-indigo-600 dark:text-indigo-500 transition-colors">SLA</span></h1>
                <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest transition-colors">Service Level Agreement & Target Resolusi</p>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-950/50 border-b border-slate-200 dark:border-slate-800 transition-colors">
                        <th class="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Nama Rencana</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Batas Waktu (Grace Hours)</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 transition-colors">
                        ...
                        <tr class="hover:bg-indigo-500/[0.02] transition-colors group">
                            ...
                            <td class="px-6 py-6 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    <span class="text-sm font-bold text-slate-700 dark:text-slate-300 transition-colors">{{ $item->grace_hours }} Jam</span>
                                    <span class="text-[10px] font-medium text-slate-400 dark:text-slate-500 transition-colors">({{ round($item->grace_hours / 24, 1) }} Hari Kerja)</span>
                                </div>
                            </td>
                            <td class="px-6 py-6 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end items-center space-x-2">
                                    <a href="{{ route('admin.sla.edit', $item) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.sla.destroy', $item) }}" class="inline"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center text-slate-500 text-xs italic font-bold">Tidak ada data SLA ditemukan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

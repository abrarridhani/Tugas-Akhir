<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase transition-colors">Tambah <span class="text-blue-600 dark:text-blue-500 transition-colors">Departemen</span></h1>
        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest transition-colors">Inisialisasi Sektor/Unit Kerja Baru</p>
    </div>
    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-8 relative overflow-hidden group shadow-sm dark:shadow-2xl transition-colors">
            ...
            <form method="POST" action="{{ route('admin.departments.store') }}" class="relative z-10 space-y-6">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <select name="is_public" id="is_public" required
                        ...
                        <option value="1" {{ old('is_public', true) ? 'selected' : '' }} class="bg-white dark:bg-slate-900">PUBLIC (Terlihat oleh Pelapor)</option>
                        <option value="0" {{ !old('is_public', true) ? 'selected' : '' }} class="bg-white dark:bg-slate-900">PRIVATE (Internal Team Only)</option>
                    </select>
                    @error('is_public')
                        <p class="mt-1 text-[10px] font-bold text-red-500 uppercase tracking-tight">{{ $message }}</p>
                    @enderror

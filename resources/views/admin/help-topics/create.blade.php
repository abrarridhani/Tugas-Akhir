<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase transition-colors">Tambah <span class="text-emerald-600 dark:text-emerald-500 transition-colors">Topik Bantuan</span></h1>
        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest transition-colors">Klasifikasi Kategori Laporan Insiden</p>
    </div>
    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-8 relative overflow-hidden group shadow-sm dark:shadow-2xl transition-colors">
            ...
            <form method="POST" action="{{ route('admin.help-topics.store') }}" class="relative z-10 space-y-8">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <select name="department_id" id="department_id" required
                        ...
                        @foreach($departments as $dept)
                            <option value="{{ $dept->id }}" {{ old('department_id') == $dept->id ? 'selected' : '' }} class="bg-white dark:bg-slate-900">
                                {{ $dept->name }}
                            </option>
                        @endforeach
                    </select>

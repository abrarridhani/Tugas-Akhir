<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase transition-colors">Tambah <span class="text-indigo-600 dark:text-indigo-400 transition-colors">Chatbot Response</span></h1>
        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest transition-colors">Konfigurasi Automasi Interaksi Baru</p>
    </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-8 relative overflow-hidden group shadow-sm dark:shadow-2xl transition-colors">
            ...
            <form method="POST" action="{{ route('admin.chatbot-responses.store') }}" class="relative z-10 space-y-8">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="response" id="response" rows="6" required
                        ...
                        class="block w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl py-4 px-6 text-sm font-bold text-slate-900 dark:text-white placeholder-slate-300 dark:placeholder-slate-800 focus:border-indigo-500 outline-none transition-all shadow-inner transition-colors">{{ old('response') }}</textarea>
                    <div class="flex items-center space-x-2 mt-2 ml-1">
                        <svg class="w-3.5 h-3.5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <select name="match_type" id="match_type" required
                            ...
                            <option value="contains" {{ old('match_type', 'contains') === 'contains' ? 'selected' : '' }} class="bg-white dark:bg-slate-900 transition-colors">CONTAINS (Mengandung Kata)</option>
                            <option value="exact" {{ old('match_type') === 'exact' ? 'selected' : '' }} class="bg-white dark:bg-slate-900 transition-colors">EXACT (Persis Sama)</option>
                            <option value="starts_with" {{ old('match_type') === 'starts_with' ? 'selected' : '' }} class="bg-white dark:bg-slate-900 transition-colors">STARTS WITH (Dimulai Dengan)</option>
                        </select>
                        <div class="mt-3 space-y-1.5 ml-1 transition-colors">
                            <div class="flex items-center text-[9px] font-bold text-slate-400 uppercase tracking-tighter"><span class="w-1 h-1 bg-indigo-500 rounded-full mr-2"></span> Contains: Fleksibel, cocok jika ada di dalam kalimat</div>
                            <div class="flex items-center text-[9px] font-bold text-slate-400 uppercase tracking-tighter"><span class="w-1 h-1 bg-indigo-500 rounded-full mr-2"></span> Exact: Kaku, harus sama persis 100%</div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center space-x-2 mt-2 ml-1">
                            <svg class="w-3.5 h-3.5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tight transition-colors">0-100 (Semakin tinggi semakin diutamakan).</p>
                        </div>
                        @error('priority')
                            <p class="mt-1 text-[10px] font-bold text-red-500 uppercase tracking-tight">{{ $message }}</p>
                        @enderror
                ...
                <div class="pt-4 border-t border-slate-100 dark:border-slate-800/50 transition-colors">
                    <label class="relative flex items-center cursor-pointer group">
                        ...
                        <span class="ml-4 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">Aktifkan Response Segera</span>
                    </label>
                </div>

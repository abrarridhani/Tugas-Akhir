<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase transition-colors">Edit <span class="text-emerald-600 dark:text-emerald-400 transition-colors">Tanggapan Cepat</span></h1>
        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest transition-colors">Modifikasi Template: {{ $cannedResponse->title }}</p>
    </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-8 relative overflow-hidden group shadow-sm dark:shadow-2xl transition-colors">
            ...
            <form method="POST" action="{{ route('admin.canned.update', $cannedResponse) }}" class="relative z-10 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="body" id="body" rows="12" required
                        ...
                        class="block w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl py-4 px-6 text-sm font-bold text-slate-900 dark:text-white placeholder-slate-300 dark:placeholder-slate-800 focus:border-emerald-500 outline-none transition-all shadow-inner transition-colors">{{ old('body', $cannedResponse->body) }}</textarea>

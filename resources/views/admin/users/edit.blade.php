<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase transition-colors">Modifikasi <span class="text-blue-600 dark:text-blue-500 transition-colors">Profil User</span></h1>
        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest transition-colors">Update Otoritas & Informasi: {{ $user->name }}</p>
    </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-8 relative overflow-hidden group shadow-sm dark:shadow-2xl transition-colors">
            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="relative z-10 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required
                            class="block w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl py-3.5 px-5 text-sm font-bold text-slate-900 dark:text-white placeholder-slate-300 dark:placeholder-slate-800 focus:border-blue-500 outline-none transition-all shadow-inner transition-colors">
                        @error('name')
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="organization_id" id="organization_id"
                            ...
                            @foreach($organizations as $org)
                                <option value="{{ $org->id }}" {{ old('organization_id', $user->id_organisasi) == $org->id ? 'selected' : '' }} class="bg-white dark:bg-slate-900 transition-colors">
                                    {{ $org->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="bg-blue-500/[0.03] dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/10 rounded-2xl p-6 space-y-6 transition-colors">
                    <div class="flex items-center space-x-2 mb-2">
                        <svg class="w-4 h-4 text-blue-600 dark:text-blue-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                        <h4 class="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.2em] transition-colors">Keamanan Kredensial</h4>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2">
                            ...
                            <input type="password" name="password" id="password"
                                class="block w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl py-3.5 px-5 text-sm font-bold text-slate-900 dark:text-white placeholder-slate-300 dark:placeholder-slate-800 focus:border-blue-500 outline-none transition-all shadow-inner transition-colors"
                                placeholder="••••••••">
                            <p class="text-[9px] font-bold text-slate-400 dark:text-slate-600 ml-1 italic transition-colors">Kosongkan jika tidak ingin mengubah</p>
                            @error('password')
                                <p class="mt-1 text-[10px] font-bold text-red-500 uppercase tracking-tight">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($roles as $role)
                            <label class="relative flex cursor-pointer group">
                                <input type="radio" name="role" value="{{ $role->name }}" {{ old('role', $user->roles->first()?->name) == $role->name ? 'checked' : '' }} class="peer sr-only">
                ...
                <div class="flex items-center justify-end space-x-4 pt-8 border-t border-slate-100 dark:border-slate-800/50 transition-colors">
                    ...
                    <button type="submit" 
                        class="px-12 py-3.5 bg-gradient-to-r from-emerald-600 to-blue-600 hover:from-emerald-500 hover:to-blue-500 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] transition-all transform hover:-translate-y-1">
